...
const CartProductItem = ({ productId, image, name, price, stock, maxValue, options, ordering, cartItemId, index }: cartProductType) => {    
  ...
  return (
<div className="py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4 cursor-pointer hover:bg-slate-50">
  <div className="flex items-center gap-6">
    <input
      type="checkbox"
      checked={ordering}
      onChange={() => handleAddToOrder({ cartItemId, dispatch, userData, index, ordering })}
    />
      ...
      <div className="flex items-center gap-5 mt-2">
        ...
        {options &&
          Object.entries(options).map((item, index) => (
            <p className="px-2 sm:px-3 sm:py-1 border bg-slate-50 hover:bg-slate-100" key={index}>
              {`${cartNames[item[0]]}: ${item[1]}`}
            </p>
          ))}
      </div>
    </div>
